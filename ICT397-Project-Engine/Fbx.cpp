...
glm::mat4 Fbx::to_glm(aiMatrix4x4t<float> m)
{
...
glm::vec3 Fbx::to_glm(aiVector3t<float> v) {
    return glm::vec3{ v.x, v.y, v.z };
}


glm::quat Fbx::to_glm(aiQuaterniont<float> q) {
    return glm::quat{ q.w, q.x, q.y, q.z };
}


void Fbx::loadModel(std::string const& path)
{
...
void Fbx::processNode(aiNode* node, const aiScene* scene, glm::mat4 transform)
{
...
Mesh Fbx::processMesh(aiMesh* mesh, const aiScene* scene, glm::mat4 transform)
{
...
std::vector<TextureMesh> Fbx::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
    std::vector<TextureMesh> textures;
