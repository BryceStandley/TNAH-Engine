...
void main()
{
	...
	uint cascadeIndex = uint(0);

	const uint SHADOW_MAP_CASCADE_COUNT = uint(4);
	for (uint i = uint(0); i < SHADOW_MAP_CASCADE_COUNT - uint(1); i++)
	{
		if (Input.ViewPosition.z < u_CascadeSplits[i])
			cascadeIndex = i + uint(1);
	}
