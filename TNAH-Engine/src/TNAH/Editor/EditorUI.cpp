	...
	void EditorUI::DrawComponentProperties(GameObject& object, const bool& addComponents)
	{
		...
		if (object.HasComponent<RigidBodyComponent>())
		{
			if (ImGui::TreeNode("Rigidbody"))
			{
				...
				if (rb.Body->GetBodyType() == Physics::BodyType::Static)
				{
					ImGui::Text("Body Type: Static");
					if (ImGui::Button("Make Dynamic"))
					{
						rb.Body->SetBodyType(Physics::BodyType::Dynamic);
					}
				}
				else
				{
					...
					{
						rb.Body->SetBodyType(Physics::BodyType::Static);
					}
				}
				if (Application::Get().GetDebugModeStatus())
				{
					ImGui::BulletText("ID: %d", rb.Body->m_ID);
					ImGui::Checkbox("Force Sleep", &rb.Body->m_Sleep);

					//Render all aspects of the RB
					if (ImGui::TreeNode("Mass##RB"))
					{
						if (rb.Body->GetBodyType() == Physics::BodyType::Dynamic)
					...
					else
					{
						...
						for (auto& col : rb.Body->m_Colliders)
						{
							...
							if (ImGui::CollapsingHeader(std::string("Attachment: " + std::to_string(count)).c_str(), ImGuiTreeNodeFlags_Bullet))
							{
								...
								switch (col->m_Type)
								{
								case Physics::Collider::ColliderType::Box:
								{
									ImGui::Text("Type: Box");
									if (Application::Get().GetDebugModeStatus())
									{
								...
								case Physics::Collider::ColliderType::Sphere:
								{
								...
								case Physics::Collider::ColliderType::Capsule:
								{
