...
namespace tnah {
	Skybox::Skybox()
	{
		...
		{
			TNAH_FATAL("Skybox lua script not found!");
		}
		else
		{
			TNAH_INFO("Skybox Lua script loaded");
			std::string right;
			std::string left;
			std::string top;
			std::string bottom;
			...
			tnah::Shader s(vertShader.c_str(), fragShader.c_str());
			skyShader = s;
		}
	...
	Skybox::Skybox(std::string right, std::string left, std::string top, std::string bottom, std::string front, std::string back, std::string vertexShader, std::string fragmentShader)
	{
		...
		cubeFaces.emplace_back(front);
		cubeFaces.emplace_back(back);
		tnah::Shader s(vertexShader.c_str(), fragmentShader.c_str());
		skyShader = s;
		SetSkyVerts();
	}
